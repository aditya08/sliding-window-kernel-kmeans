...
int test_kernel(const KernelType &kernel_type, const T tol = 1e-4) {
    ...
    if (kernel_type == KernelType::LINEAR) {
        ...
        kern = std::make_unique<TanhKernel<T>>(0.1f, 0.0f);
    } else if (kernel_type == KernelType::POLYNOMIAL) {
        kern = std::make_unique<PolynomialKernel<T>>(1.f, 0.0f, 3);
    } else {
    ...
    std::unordered_map<KernelType, T*> ref_solutions = {
        ...
        {KernelType::RBF, new T[25]{1.000000000000000,0.862829792289629,0.536523597027506,0.645687031707374,0.419129145931922,0.862829792289629,1.000000000000000,0.562817657302193,0.575172041669462,0.470158273409122,0.536523597027506,0.562817657302193,1.000000000000000,0.528904987722155,0.520458917301376,0.645687031707374,0.575172041669462,0.528904987722155,1.000000000000000,0.332755858490844,0.419129145931922,0.470158273409122,0.520458917301376,0.332755858490844,1.000000000000000}},
    ...
    std::cout << "Computing Kernel Matrix for kernel type " << kernel_type << "\n";
    kern->compute_kernel_matrix(m, m, n, A, m, A, m, C, m);
    T* ref_C = ref_solutions[kernel_type];
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < m; j++) {
            // std::cout << C[i * m + j] << " ";
            if (std::abs(C[i * m + j] - ref_C[i * m + j]) > tol) {
                std::cerr << "Test failed for kernel type: " << kernel_type << " at (" << i << ", " << j << "). Expected: " << ref_C[i * m + j] << ", Got: " << C[i * m + j] << std::endl;
                return EXIT_FAILURE;
            }
        }
        // std::cout << std::endl;
...
int main(int argc, char** argv) {
    ...
    for (auto kernel_type : {KernelType::LINEAR, KernelType::RBF, KernelType::TANH, KernelType::POLYNOMIAL}) {
        ...
        int double_precision_test = test_kernel<double>(kernel_type, 1e-8);
